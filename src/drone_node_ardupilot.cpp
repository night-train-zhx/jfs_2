		...
		else
		{
			...
			if((abs(current_pose.pose.position.x-transformed_pt.point.x)<=0.05)&&(abs(current_pose.pose.position.y-transformed_pt.point.y)<=0.05)&&(abs(current_pose.pose.position.z-transformed_pt.point.z)<=0.05))
			{
				...
				if(red_count<4)
				{
					...
					ledstate.data=true;
					led_pub.publish(ledstate);
					for (int i = 0; i < 200; i++) //5s,point_fixing
					{
						geometry_msgs::Point err;
						geometry_msgs::Point out;
						err.x = current_point.y; 
						err.y = current_point.x;
						...
						//set_pose_body(out.x, out.y,0 , 0);//set_pose_body(out.x, out.y, 1.4 - current_pose.pose.position.z, 0);
//						ROS_INFO("fixing...");
						ROS_INFO("outx=%f,outy=%f",out.x,out.y);
						ros::spinOnce();
	...
	for (int i = 0; i < 500; i++) //5s,wait before landing
	{
		ros::spinOnce();
		ros::Duration(0.01).sleep();
	}
